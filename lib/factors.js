...
/* ── NOTES — v2 (uniformity + impervious) ──────────────────────────────────────
 ...
 *   EnviroAtlas communities — swap canopy/park to it for one uniform vintage and
 *   add impervious → coverage 1.0. Sandy Springs (likely not a community) keeps
 *   the cited local canopy + ParkServe park.
 * - Walkability: city-precise = filter SLD block groups by the city boundary
 *   polygon (spatial query) rather than whole counties.
 * - Impervious via NLCD needs the relocated MRLC ImageServer URL (old paths 404'd)
 *   + computeStatisticsHistograms over the city polygon.
 */
